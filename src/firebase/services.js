...
export default class FirebaseServices {
            ...
            this.productsCollection.onSnapshot(querySnapshot => {
                ...
                querySnapshot.forEach((doc) => {
                    ...
                    products.push({
                        ...
                        category, description, name, brand,
                        picURL, price, quantity, remaining,  
                    });
                });
    ...
    getWishlist = (userId) => {
        return new Observable(observer => {
            this.wishlistCollection.where("user_id", "==", userId).onSnapshot(querySnapshot => {
                const products = [];
                const productKeys = [];
                querySnapshot.forEach(doc => {
    ...
    getUser = (userId) => {
        return new Observable(observer => {
            this.usersCollection.where(firebase.firestore.FieldPath
                .documentId(), "==", userId)
